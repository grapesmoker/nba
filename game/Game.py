...
class Game:
    ...
    def calc_events_shot_clock(self):
        ...
        for q in periods:
            ...
            shot_clock = 24.0
            for i, event in enumerate(plays):
                if event.is_jump_ball:
                    team = self.player_team(event.players[1])
                    event.possession = team
                elif event.is_field_goal_made:
                    team = self.player_team(event.shot_made_by)
                    t_diff = event.time_diff(plays[i-1])
                    event.shot_clock = t_diff
                    event.possession = self.opponent(team)
                    shot_clock -= t_diff
                elif event.is_field_goal_missed:
                    # team = self.player_team(event.shot_missed_by)
                    t_diff = event.time_diff(plays[i-1])
                    event.shot_clock = t_diff
                elif event.is_dreb or event.is_oreb:
                    reb_team = self.player_team(event.rebounded_by)
                    event.possession = reb_team
                    event.shot_clock = 24.0
                elif event.is_turnover:
                    pass
                elif i > 0 and event.play_time > plays[i-1].play_time:
                    t_diff = event.play_time - plays[i-1].play_time
